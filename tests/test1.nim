...
proc load_data(fn: string): Table[string, string] =
...
test "tokenizer positions":
  var f = open(data_file_path("pos.forms.cfg.txt"))
  var positions: seq[seq[int]] = @[]
  for line in f.lines:
    var parts = line.split(' ').map(parseInt)
    positions.add(parts)
  f.close()
  var fs = newFileStream(data_file_path("forms.cfg"))
...
test "decoder":
  var fs = newFileStream(data_file_path("all_unicode.bin"))
...
test "tokenizer data":
  var cases = load_data(data_file_path("testdata.txt"))
...
test "json":
  var path = data_file_path("forms.conf")
