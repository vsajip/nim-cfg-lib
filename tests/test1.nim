...
test "tokenizer":
  ...
  t = makeTokenizer("")
  var tk = t.getToken
  check(tk.value.kind == EOF)
  tk = t.getToken
  check(tk.value.kind == EOF)

  t = makeTokenizer("# a comment\n")
  tk = t.getToken()
  check(tk.value.kind == Newline)
  check(tk.text == "# a comment")
  check(EOF == t.getToken().value.kind)

  t = makeTokenizer("foo")
  tk = t.getToken()
  check(tk.value.kind == Word)
  check(tk.text == "foo")
  check(EOF == t.getToken().value.kind)
  ...
  if false: # TODO below
    t = makeTokenizer("'foo'")
    tk = t.getToken()
    check(StringToken == tk.value.kind)
    check("'foo'" == tk.text)
    ...
    tk = t.getToken()
    check(FloatNumber == tk.value.kind)
    ...
    tk = t.getToken()
    check(UnsignedNumber == tk.value.kind)
